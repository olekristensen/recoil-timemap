...
//------------------------------------------------------------------
TextFontHolder::TextFontHolder(string file){
	bLoadedOk = false;
	
	loadFont(file, 20, true, true);
	shader.loadShader("Invert");
	shader.printActiveUniforms();
	shader.printActiveAttributes();
}

TextFontHolder::TextFontHolder(string file, int _fontSize){
	bLoadedOk = false;
...
void TextFontHolder::drawChar(int c, float x, float y) {
	...
	//	shader.setUniform("tex",(int)texNames[cu]);
	//	shader.setUniform("foreground",(float)1.0,(float)1.0,(float)1.0);
	//	shader.setUniform("background",(float)0.0,(float)0.0,(float)0.0);
