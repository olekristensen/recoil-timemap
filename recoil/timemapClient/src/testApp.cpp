...
//	cout<<text->getNumberLetters()<<endl;
	for(int i=0; i<numLetters; i++){
		...
		btCollisionShape* fallShape = new btBoxShape(btVector3((text->getLetter(i)->getWidth()-2)/2.0,(l->getFont()->getHeight(l->getLetter())/2.0),(20)/2.0));
...
//	cout<<" --- "<<endl;
	int numLetters = text->getNumberLetters();
	dynamicsWorld->stepSimulation((ofGetElapsedTimeMillis()-millisForUpdate)/1000.0f, 20, btScalar(1.)/btScalar(400.));
//	cout << ((ofGetElapsedTimeMillis()-millisForUpdate)/1000.0) << " < " << 20 * (1.0/400.0) << endl;
	millisForUpdate = ofGetElapsedTimeMillis();
